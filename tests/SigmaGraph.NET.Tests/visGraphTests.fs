...
let visGraphTests =
    testList "VisGraph" [
        ...
        testCase "WithCircularLayout_Test_V" <| fun () ->
            ...
            Expect.equal layoutType "SigmaGraph.NET.Layout+Circular" $"The Layout type was expected to be -SigmaGraph.NET.Layout+Circular- but it is {layoutType}"  
        testCase "WithRandomLayout_Test_V" <| fun () ->
            ...
            Expect.equal layoutType  "SigmaGraph.NET.Layout+Random" $"The layout was expected to be -SigmaGraph.NET.Layout+Random- but it is {layoutType}"    
        ...
        testCase "WithForceAtlas2_Test_V" <| fun () ->
            ...
            Expect.equal appliedLayout "FA2 SigmaGraph.NET.FA2Options" $"The Layout was expected to be -FA2 SigmaGraph.NET.FA2Options- but is {appliedLayout}"
        testCase "WithNoverlap_Test_V" <| fun () ->
            ...
            Expect.equal appliedLayout "Noverlap SigmaGraph.NET.NoverlapOptions" $"The Layout was expected to be -Noverlap SigmaGraph.NET.NoverlapOptions- but it is {appliedLayout}."
